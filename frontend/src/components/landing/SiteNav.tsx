...
export default function SiteNav() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-sm">
      <div className="flex items-center justify-between px-4 sm:px-6 py-[15px] max-w-6xl mx-auto">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {NAV_LINKS.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="text-sm font-medium text-slate-600 dark:text-slate-300 transition-colors hover:text-pcblue"
            >
              {t(l.key)}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/login"
            className="btn hidden sm:inline-flex bg-gradient-to-r from-[#4361ee] to-[#3953af] text-white shadow-sm hover:brightness-110"
          >
          ...
          <nav className="max-w-6xl mx-auto px-4 py-3 flex flex-col gap-1">
            {NAV_LINKS.map((l) => (
              <Link
                ...
                className="rounded-lg px-3 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-pcblue"
              >
                {t(l.key)}
              </Link>
            ))}
            <Link
              href="/login"
              onClick={() => setOpen(false)}
              className="btn justify-center mt-1 bg-gradient-to-r from-[#4361ee] to-[#3953af] text-white shadow-sm hover:brightness-110"
            >
              {t("landing_login_button")}
            </Link>
          </nav>
