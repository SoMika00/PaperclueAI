      ...
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        {active ? (
          ...
          <div className="mt-8 grid sm:grid-cols-2 gap-4">
            {PLANS.map((p) => (
              <div key={p.id} className="rounded-2xl border border-line bg-paper dark:bg-dark-surface dark:border-dark-line p-6 shadow-card flex flex-col">
                <div className="text-sm font-semibold text-ink dark:text-dark-ink">{p.name}</div>
                <div className="mt-2 flex items-baseline gap-1">
                  <span className="font-serif text-3xl font-bold text-ink dark:text-dark-ink">{p.price}</span>
                  <span className="text-xs text-inkmut dark:text-dark-inkmut">{p.caption}</span>
                </div>
                <ul className="mt-4 space-y-2 flex-1">
                  {FEATURES.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-[13px] text-ink/85 dark:text-dark-ink">
                ...
                <button
                  ...
                  disabled={!!busy}
                  className="btn btn-primary mt-5 w-full justify-center py-2.5 text-[15px]"
                >
