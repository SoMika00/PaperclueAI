  ...
  return (
    <div className="h-full overflow-y-auto panel-scroll">
      <div className="max-w-3xl mx-auto px-8 py-8">
        <h1 className="font-serif text-xl font-semibold">{t("version_history_title")}</h1>
        <p className="text-sm text-inkmut mt-0.5 mb-6">
          {t("version_history_subtitle")}
        </p>
        ...
        <div className="flex flex-col gap-3">
          {(versions || []).map((v) => (
            <div key={v.id} className="card p-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-semibold">{t("version_number_label")} {v.number}</span>
          ...
          {versions !== null && versions.length === 0 && (
            <div className="text-sm text-inkmut">{t("no_versions_full")}</div>
          )}
        </div>
      </div>
    </div>
