...
def run_format(task_id: str, ms_id: str, journal_id: str):
    ...
    try:
        ...
        db.add(EvidenceItem(
            ...
            source_ref={
                "page": 1, "quote": "", "section": "Abstract",
                "journal_id": journal_id,
                "abstract_after": rewrite.get("abstract_after"),
                "restructure_plan": rewrite.get("restructure_plan") or [],
                "added_statements": rewrite.get("added_statements") or [],
            },
...
def export_docx(ms_id: str, journal: str = "scientific-reports", db=Depends(get_db),
    ...
    latest_format = (db.query(EvidenceItem)
                     .filter_by(manuscript_id=ms_id, kind="format")
                     .order_by(EvidenceItem.created_at.desc()).all())
    format_data = next((e.source_ref or {} for e in latest_format
                        if (e.source_ref or {}).get("journal_id") == journal), {})
    ...
    for name in j["section_order"]:
        text = None
        if name == "Abstract" and format_data.get("abstract_after"):
            text = format_data["abstract_after"]
        for sname, stext in sections.items():
            if text is None and sname.lower().startswith(name.lower()[:6]):
                text = stext
                break
        if text is None and name == "Data Availability" and format_data.get("added_statements"):
            text = "\n\n".join(format_data["added_statements"])
