...
def get_ms(db, ms_id: str, user_id: str) -> Manuscript:
    ms = db.get(Manuscript, ms_id)
    if not ms or ms.tenant_id != settings.tenant_id or ms.user_id != user_id:
        raise HTTPException(404, "Manuscript not found")
    return ms


@router.get("/manuscripts")
def list_manuscripts(db=Depends(get_db), current_user: dict = Depends(get_current_user)):
...
def get_manuscript(ms_id: str, db=Depends(get_db), current_user: dict = Depends(get_current_user)):
    return manuscript_out(get_ms(db, ms_id, current_user["user_id"]), full=True)


@router.delete("/manuscripts/{ms_id}")
def delete_manuscript(ms_id: str, db=Depends(get_db), current_user: dict = Depends(get_current_user)):
...
def ingest_status(ms_id: str, db=Depends(get_db), current_user: dict = Depends(get_current_user)):
    ms = get_ms(db, ms_id, current_user["user_id"])
...
def get_pdf(ms_id: str, db=Depends(get_db), current_user: dict = Depends(get_current_user)):
    ms = get_ms(db, ms_id, current_user["user_id"])
    return FileResponse(ms.file_path, media_type="application/pdf")


@router.get("/manuscripts/{ms_id}/references")
def list_references(ms_id: str, db=Depends(get_db), current_user: dict = Depends(get_current_user)):
...
def list_issues(ms_id: str, db=Depends(get_db), current_user: dict = Depends(get_current_user)):
    get_ms(db, ms_id, current_user["user_id"])
...
def list_evidence(ms_id: str, db=Depends(get_db), current_user: dict = Depends(get_current_user)):
    get_ms(db, ms_id, current_user["user_id"])
...
def list_versions(ms_id: str, db=Depends(get_db), current_user: dict = Depends(get_current_user)):
    get_ms(db, ms_id, current_user["user_id"])
